...
The locale plugin catalogues ten Indic languages — Hindi (`hi`), Bengali (`bn`), Marathi (`mr`), Telugu (`te`), Tamil (`ta`), Gujarati (`gu`), Kannada (`kn`), Malayalam (`ml`), Punjabi (`pa`), Odia (`or`) — with `LOCALE_IDS` at `['en', 'hi']`. Hindi ships a translated dictionary in every client namespace (~1,230 keys across 32 dictionaries); the typed `Record<BuiltInLocaleId, …>` register overload compiler-enforces that completeness, so a namespace without Hindi fails typecheck. The remaining nine languages are registered through `ctx.effect(() => locale.addLanguage({ id, label, fallback: 'en' }))` with no dictionaries yet, so per-key lookup falls through the existing fallback chain to English. Display order is English, Hindi, then the nine in descending speaker population. Where Hindi has no natural translation, the copy transliterates into Devanagari (letter-name acronyms such as एपीआई, technical badges such as सिस्टम); functional tokens stay verbatim — slash-command names (`/plan`, `compact`), key caps (`Tab`, `Cmd/Ctrl+Enter`), file-format names, unit syntax users type (`px`, `K`/`M` suffixes), URLs, and file paths. Browser language detection, `<html lang>` sync, and `locale.preference` persistence work unchanged because they already operate on registered ids.

## Coverage

Hindi dictionaries landed namespace by namespace through the single-locale `register(ns, 'hi', dict)` form (no `LOCALE_IDS` change, tree green throughout), then the flip converted every call site to the typed `{ en, hi }` form. The flip itself caught one missed namespace (`session-log-download`) plus two fixture runtimes at compile time, which is the enforcement working as designed.
