...
describe('LocaleRuntime', () => {
  beforeEach(() => {
    // An unshipped browser language is the baseline: with no zh dictionary the
    // service opens English and these specs stage anything else explicitly.
    stubLanguages('zh-CN')
  })
  ...
  it('falls through to the common vocabulary after the namespace misses (production keys)', () => {
    ...
    // ja is active and `ns` has no ja dictionary at all: the en fallback answers.
  ...
  it('interpolates {name} params and leaves unknown placeholders intact', () => {
    const { svc } = make()
    svc.register('ns', 'en', { greet: 'Hello, {name}! #{n}', partial: '{known} and {unknown}' })
    const t = svc.bind('ns')
    expect(t('greet', { name: 'world', n: 2 })).toBe('Hello, world! #2')
  ...
  it('setLocale writes through the scope and republishes only on a real change', () => {
    ...
    expect(host.set).toHaveBeenCalledWith('preference', 'fr')
    ...
    expect(host.set).toHaveBeenLastCalledWith('preference', 'fr')
  })
  ...
  it('persists an explicit pick of the provisional locale, so a shared DSH home agrees', () => {
    ...
    // durable, or another browser sharing the home would disagree.
    stubLanguages('fr-FR')
    const host = stubSettingsScope<LocaleSettings>()
    const { svc } = make(host)
  ...
  it('opens provisionally in the browser language, matching regional variants on their primary subtag', () => {
    ...
    // Chinese is not shipped: the browser's zh tags fall through to English.
  ...
  it('serves English as both the opening locale and the dictionary fallback', () => {
    ...
    // misses. Safe to share only because English is the one shipped locale and
    // therefore the terminal language of every fallback chain.
    expect(FALLBACK_LOCALE).toBe('en')
    vi.stubGlobal('window', undefined)
    const { svc } = make()
    // A key present only in en resolves for a ja reader through the fallback.
    svc.addLanguage({ id: 'ja', label: '日本語', fallback: 'en' })
    svc.register('ns', 'ja', {})
    ...
    // The reverse no longer resolves: a ja-only key is unreachable from en, so
    // the key itself surfaces (fail loud) rather than silently rendering ja.
    svc.register('ns2', 'ja', { onlyJa: '日本語のみ' })
    svc.register('ns2', 'en', {})
    svc.setLocale('en')
    expect(svc.bind('ns2')('onlyJa')).toBe('onlyJa')
  })

  it('starts with exactly the shipped locales', () => {
    const { svc } = make()
