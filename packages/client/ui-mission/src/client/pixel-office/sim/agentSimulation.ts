...
import { astar2D } from '../pathfinding.ts'
import { buildPixelNavGrid, tileCenter } from '../map/navGrid.ts'
import {
  PIXEL_TILE_SIZE,
...
export const createPixelSimulation = (map: PixelOfficeMap): PixelSimulation => {
  ...
  const pickRandomIdleStation = (agent: InternalAgentState): PixelStation | null => {
    ...
    return pool[Math.min(pool.length - 1, Math.floor(agent.rng() * pool.length))] ?? null
  }
  ...
  const tick = ({
    ...
    for (const id of Object.keys(state.agents)) {
      if (!inputIds.has(id)) {
        Reflect.deleteProperty(state.agents, id)
    ...
    const poses = inputs.flatMap((input) => {
      const agent = state.agents[input.id]
      return agent ? [poseFor(agent)] : []
    })
