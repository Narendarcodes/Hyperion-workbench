import { OBJECT_FOOTPRINT, PIXEL_TILE_SIZE, type PixelObjectKind, type PixelSprite } from '../types.ts'
import { LED_GREEN, NEAR_BLACK, PAPER_SHADE, PAPER_WHITE } from './palette.ts'
import {
  fillRect,
  gridRows,
  hLine,
...
import { makeSprite } from './sprite.ts'
...
function deskMonitor(): PixelSprite {
  // ponytail: enlarged workstation (taller monitor + full desk dressing);
  // width stays at the 2-tile footprint so nav and placements are untouched.
  const g = gridFor('desk_monitor', 76)
  drawDeskBody(g, 40)
  // Monitor panel with a rounded bezel (taller screen, more code lines).
  roundRect(g, 14, 0, 36, 26)
  fillRound(g, 14, 0, 36, 26, 'm')
  fillRect(g, 17, 3, 30, 20, 'M')
  ...
  setPx(g, 23, 4, '1')
  hLine(g, 28, 4, 10, 'S')
  // Nine colored code lines with varied indents.
  hLine(g, 19, 7, 4, 'P')
  hLine(g, 24, 7, 9, 'c')
  hLine(g, 21, 9, 5, 'c')
  hLine(g, 27, 9, 6, '1')
  ...
  fillRect(g, 30, 25, 4, 15, 'm')
  fillRect(g, 26, 40, 12, 3, 'm')
  hLine(g, 26, 41, 12, 'V')
  // Keyboard with key rows and a space bar.
  outlineRect(g, 15, 47, 20, 9, 'o')
  fillRect(g, 16, 48, 18, 7, 's')
  for (let ky = 49; ky <= 53; ky += 2) {
    for (let kx = 17; kx <= 31; kx += 3) hLine(g, kx, ky, 2, 'S')
  }
  hLine(g, 21, 53, 8, 'S')
  // Mouse pad + mouse.
  fillRect(g, 38, 48, 12, 9, 'v')
  trimCorners(g, 38, 48, 12, 9)
  fillRect(g, 42, 50, 4, 6, 's')
  vLine(g, 43, 53, 2, 'S')
  setPx(g, 42, 50, 'n')
  // Mug with steam beside the stand.
  fillRect(g, 40, 41, 6, 6, 'r')
  hLine(g, 40, 41, 6, '9')
  hLine(g, 41, 42, 3, 'h')
  outlineRect(g, 46, 42, 2, 3, '9')
  setPx(g, 42, 39, '*')
  setPx(g, 44, 38, '*')
  setPx(g, 43, 37, '*')
  // Small potted plant on the right.
  fillRect(g, 52, 48, 7, 6, '2')
  hLine(g, 52, 48, 7, 'e')
  hLine(g, 52, 53, 7, '3')
  fillRect(g, 52, 42, 7, 6, 'g')
  trimCorners(g, 52, 42, 7, 6)
  setPx(g, 53, 42, 'l')
  setPx(g, 54, 43, 'l')
  setPx(g, 52, 44, 'l')
  setPx(g, 57, 46, 'G')
...
function chair(): PixelSprite {
  // ponytail: enlarged chair (taller backrest + headrest); 1-tile footprint kept.
  const g = gridFor('chair', 44)
  drawShadow(g, 5, 41, 22)
  ...
  setPx(g, 19, 39, 'o')
  setPx(g, 20, 40, 'o')
  fillRect(g, 3, 38, 3, 3, 'V')
  fillRect(g, 26, 38, 3, 3, 'V')
  fillRect(g, 9, 40, 3, 3, 'V')
  fillRect(g, 20, 40, 3, 3, 'V')
  fillRect(g, 14, 32, 4, 6, 'V')
  vLine(g, 14, 32, 6, 'v')
  // Backrest seen from behind: headrest, highlight band, bolsters, lumbar seam.
  roundRect(g, 5, 0, 22, 22)
  fillRound(g, 5, 0, 22, 22, 'I')
  fillRect(g, 8, 1, 16, 4, 'j')
  fillRect(g, 10, 6, 12, 2, 'j')
  vLine(g, 6, 4, 15, 'i')
  vLine(g, 25, 4, 15, 'i')
  setPx(g, 9, 11, 'i')
  hLine(g, 10, 12, 12, 'i')
  ...
  roundRect(g, 3, 20, 26, 10)
  fillRound(g, 3, 20, 26, 10, 'i')
  hLine(g, 6, 21, 20, 'j')
  fillRect(g, 4, 26, 24, 3, 'V')
  // Armrests hugging the seat sides.
  outlineRect(g, 0, 16, 4, 11, 'o')
  fillRect(g, 1, 17, 2, 9, 'v')
  hLine(g, 1, 17, 2, 's')
  outlineRect(g, 28, 16, 4, 11, 'o')
  fillRect(g, 29, 17, 2, 9, 'v')
  hLine(g, 29, 17, 2, 's')
...
function bookshelf(): PixelSprite {
  ...
  for (let i = 0; i < 10; i += 1) {
    const h = topHeights[i] ?? 10
    const c = topColors[i] ?? 'r'
    fillRect(g, 6 + i * 4, 28 - h, 3, h, c)
  ...
  for (let i = 0; i < 8; i += 1) {
    const h = lowHeights[i] ?? 10
    const c = lowColors[i] ?? 't'
    fillRect(g, 6 + i * 4, 45 - h, 3, h, c)
