...
import { CAP_YELLOW, CHAR_WHITE, FALLBACK_SHIRT } from './palette.ts'
import { fillRect, gridRows, hLine, makeGrid, setPx, vLine, type Grid } from './grid.ts'
import { makeSprite, spriteWidth } from './sprite.ts'
...
function resolveAppearance(look: CharacterLook): Appearance {
  ...
  if (look.seed.startsWith('npc-janitor')) {
    const janitorSkin = SKIN_TONES[2] ?? '#fcd3b6'
    const janitorHair = HAIR_COLORS[4] ?? { base: '#4a5568', highlight: '#718096', dark: '#2d3748' }
    const janitorPants = PANTS_COLORS[0] ?? '#334155'
    const janitorCap = CAP_COLORS[0] ?? { base: '#e8c832', brim: '#c4a622' }
    return {
      skin: janitorSkin,
      skinShade: darkenHex(janitorSkin, 0.85),
      hair: janitorHair.base,
      hairHighlight: janitorHair.highlight,
      hairDark: janitorHair.dark,
      hairStyle: 'short',
      pants: janitorPants,
      pantsCrease: darkenHex(janitorPants, 0.72),
      shirt,
      shirtShade,
      accessory: 'none',
      capBase: janitorCap.base,
      capBrim: janitorCap.brim,
      janitorCap: true,
    }
  }
  const h = hashSeed(look.seed)
  const skin = SKIN_TONES[h % SKIN_TONES.length] ?? SKIN_TONES[0] ?? '#fcd3b6'
  const hair = HAIR_COLORS[(h >>> 3) % HAIR_COLORS.length] ?? HAIR_COLORS[0] ?? { base: '#3b2f2f', highlight: '#5c4033', dark: '#221a1a' }
  const pants = PANTS_COLORS[(h >>> 9) % PANTS_COLORS.length] ?? PANTS_COLORS[0] ?? '#334155'
  const hairStyle = HAIR_STYLES[(h >>> 6) % HAIR_STYLES.length] ?? 'short'
  const accessoryRoll = (h >>> 12) % 5
  const cap = CAP_COLORS[(h >>> 15) % CAP_COLORS.length] ?? CAP_COLORS[0] ?? { base: '#e8c832', brim: '#c4a622' }
  return {
    skin,
