...
import { resolvePlaceLabel } from './taskZones.ts'
import type {
  PixelDeskSlot,
  PixelFacing,
  PixelGroundTile,
...
export const buildHermesHqMap = (): PixelOfficeMap => {
  ...
  // Center + south.
  fillRect(ground, 'carpet_purple', 18, 30, 16, 10) // Lounge (shrunk).
  // Former game corner (removed for the Hyperion demo): neutral floor.
  fillRect(ground, 'floor_cream', 34, 25, 11, 9)
  ...
  const objects: PixelMapObject[] = [
    ...
    // Product team pods (two widened pods of four desks).
    obj('desk_monitor', 19, 12),
    obj('desk_monitor', 22, 12),
    obj('desk_monitor', 19, 16),
    obj('desk_monitor', 22, 16),
    obj('chair', 19, 13, false),
    obj('chair', 22, 13, false),
    obj('chair', 19, 17, false),
    obj('chair', 22, 17, false),
    obj('desk_monitor', 31, 12),
    obj('desk_monitor', 34, 12),
    obj('desk_monitor', 31, 16),
    obj('desk_monitor', 34, 16),
    obj('chair', 31, 13, false),
    obj('chair', 34, 13, false),
    obj('chair', 31, 17, false),
    obj('chair', 34, 17, false),
    obj('lamp', 18, 18),
    obj('lamp', 36, 18),
    ...
    obj('plant', 19, 29),
    obj('plant_tall', 24, 29),
    obj('plant', 29, 29),
    obj('plant_tall', 34, 29),
    obj('plant', 39, 29),
    ...
    // Lounge (compact, shifted south).
    obj('tv_stand', 20, 31),
    obj('sofa_h', 24, 34),
    obj('sofa_v', 22, 35),
    obj('sofa_v', 27, 35),
    obj('coffee_table', 25, 36),
    obj('rug', 24, 35, false),
    obj('ping_pong_table', 29, 38),
    obj('plant', 18, 30),
    obj('plant', 18, 38),
    obj('flower', 33, 38),

    // Former game corner: stripped for the Hyperion demo (arcade, jukebox,
    // and vending machine removed); one plant stays as decor.
    obj('plant', 34, 25),
  ...
  const desks: PixelDeskSlot[] = [
    desk('desk-a1', 19, 12, 19, 13),
    desk('desk-a2', 22, 12, 22, 13),
    desk('desk-a3', 19, 16, 19, 17),
    desk('desk-a4', 22, 16, 22, 17),
    desk('desk-b1', 31, 12, 31, 13),
    desk('desk-b2', 34, 12, 34, 13),
    desk('desk-b3', 31, 16, 31, 17),
    desk('desk-b4', 34, 16, 34, 17),
  ...
  const stations: PixelStation[] = [
    ...
    station('lounge_seat', 24, 34, 'down'),
    station('lounge_seat', 25, 34, 'down'),
    station('lounge_seat', 22, 35, 'right'),
    station('lounge_seat', 27, 35, 'left'),
    station('library', 44, 5, 'down'),
    station('library', 45, 5, 'down'),
    station('library', 44, 9, 'down'),
    station('library', 45, 9, 'down'),
    station('ping_pong', 27, 38, 'right'),
    station('ping_pong', 31, 38, 'left'),
  ...
  const zones: PixelZone[] = [
    zone('z-meeting', 'meeting', resolvePlaceLabel('z-meeting', 'Meeting room'), 6, 2, 11, 10),
    zone('z-kitchen', 'kitchen', 'Kitchen', 18, 2, 10, 7),
    zone('z-cx', 'team_pod', 'CX team', 29, 2, 10, 7),
    zone('z-reading', 'library', resolvePlaceLabel('z-reading', 'Reading nook'), 40, 2, 14, 11),
    zone('z-product', 'team_pod', resolvePlaceLabel('z-product', 'Product team'), 18, 10, 21, 20),
    zone('z-ops', 'focus', 'Ops corner', 40, 13, 14, 11),
    zone('z-phone', 'phone', resolvePlaceLabel('z-phone', 'Phone booths'), 6, 13, 11, 9),
    zone('z-gym', 'gym', resolvePlaceLabel('z-gym', 'Gym'), 6, 23, 11, 10),
    zone('z-lounge', 'lounge', 'Lounge', 18, 30, 16, 10),
