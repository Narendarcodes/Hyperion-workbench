...
The composer mic control captures a voice prompt through `MediaRecorder` and posts it to the deployment's transcription route. While recording or transcribing, a ChatGPT-style voice pill takes over the composer content area — X discard left, neutral scrolling history strip middle, stop plus blue send right — while the draft, toolbar, and Workspace-write chip unmount underneath, so overlap is structurally impossible. Stop and send both end capture and transcribe; the transcript pastes into the draft as editable text for a manual send. The stream's tracks stay owned by the recorder flow: the stop path releases them, and an unmount or session switch mid-capture stops the recorder and releases the borrowed stream.
